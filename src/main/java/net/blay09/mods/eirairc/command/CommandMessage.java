...
public class CommandMessage implements SubCommand {
    ...
    public boolean processCommand(ICommandSender sender, IRCContext context, String[] args, boolean serverSide) throws CommandException {
        ...
        ITextComponent chatComponent = MessageFormat.formatChatComponent(format, target.getConnection(), target, botUser, message, MessageFormat.Target.IRC, isEmote ? MessageFormat.Mode.Emote : MessageFormat.Mode.Message);
        if (isEmote) {
            ThemeSettings themeSettings = ConfigHelper.getTheme(target);
            TextFormatting emoteColor = ((IRCUserImpl) botUser).getNameColor();
            if (emoteColor == null) {
