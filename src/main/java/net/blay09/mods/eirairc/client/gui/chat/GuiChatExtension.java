...
public class GuiChatExtension {
    ...
    public boolean autocompletePlayernames() {
        ...
        if (chatSession.getChatTarget() == null) {
            ...
            int i = inputField.getNthWordFromPosWS(-1, inputField.getCursorPosition(), false);
            String searchName = inputField.getText().substring(i).toLowerCase();
            if (ircNamesFound == chatSession.getChatTarget()) {
                inputField.deleteFromCursor(inputField.getNthWordFromPosWS(-1, inputField.getCursorPosition(), false) - inputField.getCursorPosition());
            ...
            if (foundIRCNames.size() > 1) {
                ...
                parentScreen.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion(new TextComponentString(sb.toString()), 1);
            }
            if (!foundIRCNames.isEmpty()) {
                inputField.writeText(TextFormatting.getTextWithoutFormattingCodes(foundIRCNames.get(autoCompleteIndex++)));
            }
        }
        return false;
    }
        ...
        if (keyCode == 28 || keyCode == 156) {
            ...
            if (s.length() > 0) {
                if (!MinecraftForge.EVENT_BUS.post(new ClientChatEvent(s))) {
                    if (s.charAt(0) != '/' || ClientCommandHandler.instance.executeCommand(parentScreen.mc.player, s) != 1) {
                        parentScreen.mc.player.sendChatMessage(s);
                    }
                }
    ...
    public boolean mouseClicked(int mouseX, int mouseY, int button) {
        if (button == 0 && parentScreen.mc.gameSettings.chatLinks) {
            ITextComponent clickedComponent = parentScreen.mc.ingameGUI.getChatGUI().getChatComponent(Mouse.getX(), Mouse.getY());
            if (clickedComponent != null) {
                ClickEvent clickEvent = clickedComponent.getStyle().getClickEvent();
                if (clickEvent != null) {
